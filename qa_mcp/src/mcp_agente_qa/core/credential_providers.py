...
class EntraDeviceCodeConfigService:
    ...
    def get(self) -> EntraDeviceCodeConfig:
        if self._cached is not None:
            return self._cached

        client_id = (self._keyvault_client.get_secret(env.DEVICE_CODE_CLIENT_ID_SECRET_NAME) or "").strip()
        tenant_id = (self._keyvault_client.get_secret(env.DEVICE_CODE_TENANT_ID_SECRET_NAME) or "").strip()
